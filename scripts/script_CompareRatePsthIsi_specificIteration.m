...
specID = '17083001_10-ch12a';

% CONSTANTS
PREDBS_TIME = ppar.preDbsTime; % seconds
DBS_TIME = ppar.dbsTime; % seconds
BINS_PER_DECADE = ppar.binsPerDecade;
CURR_FUNC = 'script_CompareRatePsthIsi_specificIteration';

% ROW = 1; % use this to plot just the row of interest in the table
...
binWidth = 1; %seconds
nRecs = size(T, 1);
iRec = find(strcmp(specID, T.objectID));

    disp(['file ', num2str(iRec), ' of ', num2str(nRecs)])
