...
% NOTE after line 527 need to not proceeed, this script is just a test
...
ppar.groups.dbsFrequency.groupFreqs = {10};
ppar.groups.dbsFrequency.groupLabels = {'hz10'};
...
%     iNex
    ...
    FRposCorr(iNex,1) = FRdbs.rateCorrected;
...
isHz = Tfinal.dbsFrequency == Hz;
TspecificFreq = Tfinal(isHz,:);
data = [TspecificFreq.Hdbs1_BitpSPKperc, ...
        TspecificFreq.Hdbs2_BitpSPKperc, TspecificFreq.Hpos_BitpSPKperc];
    
figure; ax = axes;
boxplot(data)
title([ppar.subjID, ': ', num2str(Hz), ' Hz case']);
ylabel('Bits/SPIKE delta%');
% ax.XTickLabel = {'pre', 'dbs 0-30sec', 'dbs 30-60sec', 'post'};
ax.XTickLabel = {'dbs 0-30sec', 'dbs 30-60sec', 'post'};
