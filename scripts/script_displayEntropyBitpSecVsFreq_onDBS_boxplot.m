...
%     iNex
    ...
    FRdbsCorr(iNex,1) = FRdbs.rateCorrected;
    ...
    %% Calculate entropy, normalized by bit/spike
    ...
    if exist(fullPathFn, 'file')
        ...
        refT = StimTs.DBS(1);
        [spkTimesDbs] = getIntervalEvents(spkTimes, refT, SPKINTERV_DBS);
...
BitpSec = FRdbsCorr .* Hdbs;

% Append data of interest to Tselect
N = [Tselect, table(FRdbsCorr), table(Hdbs), table(BitpSec)];

% Remove any rows that contain a NaN for entropy
Tfinal = N;
Tfinal(isnan(Tfinal.Hdbs),:) = [];

% % Remove any units that have less than minimum number of trials:
