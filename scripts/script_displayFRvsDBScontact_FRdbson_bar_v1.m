...
T = Tdisp;
nCond = length(freqs);
avDbsFRz = zeros(1, nCond);
semDbsFRz = zeros(1, nCond);
for ifreq = 1:nCond
    % get sub-selection of table pertaining to current frequency
    idxFreq = strcmp(T.dbsContact,  freqs{ifreq});
    T_ifreq = T(idxFreq,:);
...
ax1.XTickLabel = {'base', 'C0', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7'};
% ax1.YLim = [-1, 1];
tit = title([ppar.subjID, ': FR during DBS']);
xlabel('DBS Electrode');
...
FRdbs = T{:, FRvar};
% grpDouble = T{:, 'dbsContact'};
% grpdbsStr = string(grpDouble);
grpdbsStr = T{:, 'dbsContact'};
...
% 1 at a time t-test comparison of each group FR distribution against
% average baseline firing rate, corrected for multiple comparisons

contLab = 'C7'
avBaseFR = mean(FR(strcmp(grpStr, 'base')));
[h, p, ci, stats] = ttest(FR(strcmp(grpStr, contLab)) - avBaseFR)
statTable(1,1) = p;
statTable(1,2) = ci(1);
statTable(1,3) = ci(2);
statTable(1,4) = stats.tstat;
statTable(1,5) = stats.df;
statTable(1,6) = stats.sd;


%% Additional rate-increase/decrease classification step for each DBS contact
...
Temp = [T, table(pValranksum), table(FRsigDiff), table(wasIncrFR), ...
...
contact = {'C0', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7'}; % Make sure to include C7 if this is Uva, Kramer up to C6 only
nFreqs = numel(contact);
catCounts = zeros(nFreqs, 3);

% Gather all counts
for iFr = 1:nFreqs
    % get subselection of table rows pertaining to frequency "iFr"
    T_byContact = Temp(strcmp(Temp.dbsContact,contact{iFr}),:);
    
    % categorize each trial into "incrH", "decrH", "noCh" (in that order), 
    % keep count
    
    catCounts(iFr,:) = tab_getVariableCounts(T_byContact, {'wasIncrFR', 'noChangeFR', 'wasDecrFR'});
...
Ttest = Temp;
[xtab, chi2, p, labels] = crosstab(Ttest.dbsContact, Ttest.FRchangeLabel)


% Chi-2 test for independence post-hoc test: FRdecr vs. notFRdecr
disp('Chi-2 test for FRdecr vs. notFRdecr');
Ttest = Temp;
...
[xtab, chi2, p, labels] = crosstab(Ttest.dbsContact, Ttest.FRsigDecr)
...
[xtab, chi2, p, labels] = crosstab(Ttest.dbsContact, Ttest.FRsigIncr)
