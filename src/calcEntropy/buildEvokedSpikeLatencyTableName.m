function name = buildEvokedSpikeLatencyTableName(pipeParams)
...
if isfield(pipeParams, 'pValAlpha')
    pVal = pipeParams.pValAlpha;
    base = num2str(round(pVal));
    pLab = [base, 'p'];
    inter = sprintf('%.2s',pVal);
    ord = str2num(inter(end));
    ordt = ord;
    while ordt > 1
        pLab = [pLab, '0'];
        ordt = ordt-1;
    ...
    pLab = [pLab, num2str(floor(pVal * 10^(ord)))];
    
    label{nStr,1} = ['HletterPVAL', pLab];
    nStr = nStr+1;
