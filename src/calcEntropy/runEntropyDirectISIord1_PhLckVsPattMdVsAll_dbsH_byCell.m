function runEntropyDirectISIord1_PhLckVsPattMdVsAll_deltaH_byCell(pipeParams)
...
% 
% dbsH_PhsLckNeu_Cswp = calcDeltaEntropyISIord1_allTableRows(R_ISI_PhsLckNeu_Cswp);
...
%% GET DBS-Entropies for ISI-based Entropy of each trial

% extract from table 1st order entropy

ordH = 1; % order of entropy that we want to extract from the table

dbsH_PhsLckNeu_Cswp = cellfun(@(v)v(ordH), R_ISI_PhsLckNeu_Cswp.H_DBS);
dbsH_NOTPhsLckNeu_Cswp = cellfun(@(v)v(ordH), R_ISI_NOTPhsLckNeu_Cswp.H_DBS);
dbsH_AllNeu_Cswp = cellfun(@(v)v(ordH), R_ISI_Cswp.H_DBS);


dbsH_PhsLckNeu_Fswp = cellfun(@(v)v(ordH),R_ISI_PhsLckNeu_Fswp.H_DBS);
dbsH_NOTPhsLckNeu_Fswp = cellfun(@(v)v(ordH),R_ISI_NOTPhsLckNeu_Fswp.H_DBS);
dbsH_AllNeu_Fswp = cellfun(@(v)v(ordH),R_ISI_Fswp.H_DBS);
...
f1 = figure;
dispDeltaH_Cswp_Boxplot(dbsH_PhsLckNeu_Cswp, R_ISI_PhsLckNeu_Cswp.dbsElectrode, Clabels_PhsLckNeu);
title('ISI-Entropy in Phase-locked Contact-Sweep trials');
[pC, tabC, statsC] = anova1(dbsH_PhsLckNeu_Cswp, R_ISI_PhsLckNeu_Cswp.dbsElectrode);
set(f1, 'Position', [168 400 560 420])

% Display boxplots & ANOVA results of NON-Phase-locked cells 
f2 = figure;
dispDeltaH_Cswp_Boxplot(dbsH_NOTPhsLckNeu_Cswp, R_ISI_NOTPhsLckNeu_Cswp.dbsElectrode, Clabels_NOTPhsLckNeu);
title('ISI-Entropy in NONPhase-locked Contact-Sweep trials');
[pC, tabC, statsC] = anova1(dbsH_NOTPhsLckNeu_Cswp, R_ISI_NOTPhsLckNeu_Cswp.dbsElectrode);
set(f2, 'Position', [753 398 560 420])

% Display boxplots & ANOVA results of ALL cells
f3 = figure;
dispDeltaH_Cswp_Boxplot(dbsH_AllNeu_Cswp, R_ISI_Cswp.dbsElectrode, Clabels_AllNeu);
title('ISI-Entropy in ALL Contact-Sweep trials');
[pC, tabC, statsC] = anova1(dbsH_AllNeu_Cswp, R_ISI_Cswp.dbsElectrode);
...
% Display boxplots & ANOVA results of Phase-locked cells 
f4 = figure;
dispDeltaH_Fswp_Boxplot(dbsH_PhsLckNeu_Fswp, convert2stringArray(R_ISI_PhsLckNeu_Fswp.dbsFrequency(:)), ... 
                        Flabels_PhsLckNeu);
title('ISI-Entropy in Phase-locked Frequency-Sweep trials');
[pF, tabF, statsF] = anova1(dbsH_PhsLckNeu_Fswp, convert2stringArray(R_ISI_PhsLckNeu_Fswp.dbsFrequency(:)));
set(f4, 'Position', [1971 416 560 420])


% Display boxplots & ANOVA results of Phase-locked cells 
f5 = figure;
dispDeltaH_Fswp_Boxplot(dbsH_NOTPhsLckNeu_Fswp, convert2stringArray(R_ISI_NOTPhsLckNeu_Fswp.dbsFrequency(:)), ... 
                        Flabels_NOTPhsLckNeu);
title('ISI-Entropy in NONPhase-locked Frequency-Sweep trials');
[pF, tabF, statsF] = anova1(dbsH_NOTPhsLckNeu_Fswp, convert2stringArray(R_ISI_NOTPhsLckNeu_Fswp.dbsFrequency(:)));
set(f5, 'Position', [2554 415 560 420])


% Display boxplots & ANOVA results of Phase-locked cells 
f6 = figure;
dispDeltaH_Fswp_Boxplot(dbsH_AllNeu_Fswp, convert2stringArray(R_ISI_Fswp.dbsFrequency(:)), ...
                        Flabels_AllNeu);
title('ISI-Entropy in ALL Frequency-Sweep trials');
[pF, tabF, statsF] = anova1(dbsH_AllNeu_Fswp, convert2stringArray(R_ISI_Fswp.dbsFrequency(:)));
