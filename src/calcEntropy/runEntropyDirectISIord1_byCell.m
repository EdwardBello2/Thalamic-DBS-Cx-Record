function [pC, tabC, statsC, pF, tabF, statsF] = runEntropyDirectISIord1_byCell(pipeParams)
% Run Run ISI-based 1st-order Entropy only Estimation for each cell labelled as 
% significantly pattern-modulated pipeline
% 
...
R_modNeurons_Cswp = R_Cswp;

% for Fsweep:
R_modNeurons_Fswp = extractModNeuronRows(R_Fswp, alpha);
R_modNeurons_Fswp = R_Fswp;
   

%% CALCULATE Delta-Entropies for ISI-based Entropy of each trial

deltaH_Cswp = calcDeltaEntropyISIord1_allTableRows(R_modNeurons_Cswp);

deltaH_Fswp = calcDeltaEntropyISIord1_allTableRows(R_modNeurons_Fswp);
...
title('ISI-Entropy change in Modulated Contact-Sweep trials');
set(gcf, 'Position', [150   438   560   420]);
...
title('ISI-Entropy change in Modulated Frequency-Sweep trials');
set(gcf, 'Position', [755   438   560   420]);


%% PERFORM 1-way ANOVA

[pC, tabC, statsC] = anova1(deltaH_Cswp, rowClabel, 'displayopt', 'off');

[pF, tabF, statsF] = anova1(deltaH_Fswp, rowFlabel, 'displayopt', 'off');
...
if pipeParams.finalFigs.save == true
    ...
    % save as .eps files
    saveas(f1, [savfPn, '\EntropyDirectISI_byCell_Cswp_', pipeParams.subjID ],'epsc');
    saveas(f2, [savfPn, '\EntropyDirectISI_byCell_Fswp_', pipeParams.subjID ],'epsc');
    
    % save as .jpg files
    saveas(f1, [savfPn, '\EntropyDirectISI_byCell_Cswp_', pipeParams.subjID ],'jpg');
    saveas(f2, [savfPn, '\EntropyDirectISI_byCell_Fswp_', pipeParams.subjID ],'jpg');
