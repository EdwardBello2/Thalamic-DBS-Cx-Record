function nexFileNEW = nexSortbyPSTH(nexFile,edgeLeft,winWidth,prethreshTime)
...
if winWidth >= (ISI/fs)
    error('winWidth is wider than median inter-stimulus interval of your event')
end
...
%     unit = 1;
    ...
    % if the unit in question has no single suspected spikes, move onto
    % next unit immediately
    if isempty(PSTHwaveforms)
        continue
    end
...
plotAllPSTH(nexFileNEW,lSamp,winSamp,prethreshSamps);
...
% initialize the outputs --  in case no suspects are found
suspectIdx = [];
suspectTSamps = [];
suspectWaveforms = [];
...
function plotAllPSTH(nexFileNEW,lSamp,winSamp,prethreshSamps)

fs = nexFileNEW.waves{1,1}.WFrequency;
dbsTimes = nexFileNEW.events{end,1}.timestamps;
dbsTSamps = round(dbsTimes*fs);
numUnits = size(nexFileNEW.waves,1);

for k = 1:numUnits
    
    spkWaveforms = nexFileNEW.waves{k,1}.waveforms';
    spkTimes = nexFileNEW.waves{k,1}.timestamps;
    spkTSamps = round(spkTimes*fs);
    ...
    [bla,psthTSamps,psthWaveforms] = gatherSuspects(spkIdx,spkTSamps,spkWaveforms,dbsTSamps,lSamp,winSamp);
    
    if isempty(psthTSamps)
         PSTH = zeros(1,winSamp);
    else
        PSTH = gatherPSTH(psthTSamps,psthWaveforms,prethreshSamps,winSamp);
    end
    ...
    spkName = nexFileNEW.waves{k,1}.name;
    ldbs = nexFileNEW.events{end,1}.name;
